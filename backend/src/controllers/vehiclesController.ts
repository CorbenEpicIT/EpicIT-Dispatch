...
export const listVehicleStock = async (vehicleId: string) => {
	const vehicle = await db.vehicle.findUnique({ where: { id: vehicleId } });
	if (!vehicle) return { err: "Vehicle not found", items: null };

	const items = await db.vehicle_stock_item.findMany({
...
export const addVehicleStockItem = async (vehicleId: string, data: unknown) => {
	try {
		...
		const invItem = await db.inventory_item.findUnique({ where: { id: parsed.inventory_item_id } });
		if (!invItem) return { err: "Inventory item not found" };

		const existing = await db.vehicle_stock_item.findUnique({
			where: { vehicle_id_inventory_item_id: { vehicle_id: vehicleId, inventory_item_id: parsed.inventory_item_id } },
		});
		if (existing) return { err: "This item is already in the vehicle's stock" };

		const item = await db.vehicle_stock_item.create({
			data: {
...
export const updateVehicleStockItem = async (vehicleId: string, itemId: string, data: unknown) => {
	try {
		const parsed = updateStockItemSchema.parse(data);

		const existing = await db.vehicle_stock_item.findFirst({
			where: { id: itemId, vehicle_id: vehicleId },
		});
		if (!existing) return { err: "Stock item not found" };

		const item = await db.vehicle_stock_item.update({
			where: { id: itemId },
			data: {
...
export const deleteVehicleStockItem = async (vehicleId: string, itemId: string) => {
	try {
		const existing = await db.vehicle_stock_item.findFirst({
			where: { id: itemId, vehicle_id: vehicleId },
		});
		...
		await db.vehicle_stock_item.delete({ where: { id: itemId } });
		return { err: "" };
	} catch (e: unknown) {
...
export const createRestockRequest = async (vehicleId: string, itemId: string, technicianId: string, data: unknown) => {
	try {
...
export const setTechnicianVehicle = async (technicianId: string, vehicleId: string | null) => {
	try {
		if (vehicleId !== null) {
			const vehicle = await db.vehicle.findUnique({ where: { id: vehicleId } });
			if (!vehicle) return { err: "Vehicle not found" };
...
export const addPartsUsed = async (visitId: string, data: unknown) => {
	try {
		const parsed = addPartsUsedSchema.parse(data);

		const stockItem = await db.vehicle_stock_item.findUnique({
			where: { id: parsed.stock_item_id },
			include: { inventory_item: true },
		});
		if (!stockItem) return { err: "Stock item not found" };

		const visit = await db.job_visit.findUnique({ where: { id: visitId } });
