...
export const listNotifications = async (technicianId: string, unreadOnly = false) => {
	const notifications = await db.technician_notification.findMany({
		where: {
...
export const markNotificationRead = async (technicianId: string, notifId: string) => {
	try {
		const notif = await db.technician_notification.findFirst({
			where: { id: notifId, technician_id: technicianId },
		});
		if (!notif) return { err: "Notification not found" };

		const updated = await db.technician_notification.update({
			where: { id: notifId },
...
export const markAllNotificationsRead = async (technicianId: string) => {
	try {
		await db.technician_notification.updateMany({
