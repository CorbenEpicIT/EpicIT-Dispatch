...
router.get("/:id/stock", async (req, res, next) => {
	try {
		const { id } = req.params;
		const result = await listVehicleStock(id);
		if (result.err) {
...
router.post("/:id/stock", async (req, res, next) => {
	try {
		const { id } = req.params;
		const result = await addVehicleStockItem(id, req.body);
		if (result.err) {
			const statusCode = result.err.includes("not found") ? 404 : 400;
			return res.status(statusCode).json(createErrorResponse(ErrorCodes.VALIDATION_ERROR, result.err));
		}
...
router.put("/:id/stock/:itemId", async (req, res, next) => {
	try {
		...
		const result = await updateVehicleStockItem(id, itemId, req.body);
		if (result.err) {
			const statusCode = result.err.includes("not found") ? 404 : 400;
			return res.status(statusCode).json(createErrorResponse(ErrorCodes.VALIDATION_ERROR, result.err));
		}
...
router.delete("/:id/stock/:itemId", async (req, res, next) => {
	try {
		const { id, itemId } = req.params;
		const result = await deleteVehicleStockItem(id, itemId);
		if (result.err) {
			const statusCode = result.err.includes("not found") ? 404 : 400;
...
router.post("/:id/stock/:itemId/restock-request", async (req, res, next) => {
	try {
		...
		const result = await createRestockRequest(id, itemId, technicianId, req.body);
		if (result.err) {
			const statusCode = result.err.includes("not found") ? 404 : 400;
			return res.status(statusCode).json(createErrorResponse(ErrorCodes.VALIDATION_ERROR, result.err));
		}
