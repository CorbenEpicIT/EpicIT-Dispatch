            ...
            <div className="bg-surface rounded-2xl shadow-md p-10 max-w-md w-full text-center">
                ...
                {status === "input" && (
                    <form onSubmit={handleSubmit} className="text-left">
                        <h2 className="text-xl font-semibold text-primary mb-2 text-center">Reset Password</h2>
                        <p className="text-secondary text-sm mb-6 text-center">Enter your new password below.</p>
                        ...
                        <div className="mb-4">
                            ...
                            <input
                                ...
                                className="w-full border border-input bg-surface-inset text-primary placeholder:text-faint rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-primary-border"
                        ...
                        {errorMessage && (
                            <p className="text-error-text text-sm mb-4">{errorMessage}</p>
                        )}

                        <button
                            type="submit"
                            className="w-full bg-primary-hover hover:bg-primary-active text-on-primary font-medium py-2 rounded-lg transition"
                        >
                            Reset Password
                        </button>
                    </form>
                ...
                {status === "loading" && (
                    <>
                        ...
                        <h2 className="text-xl font-semibold text-primary">Resetting password...</h2>
                ...
                {status === "error" && (
                    <>
                        <div className="w-14 h-14 bg-error-bg rounded-full flex items-center justify-center mx-auto mb-6">
                            <svg className="w-7 h-7 text-error-text" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                            </svg>
                        </div>
                        <h2 className="text-xl font-semibold text-primary">Reset Failed</h2>
