...
import { Search, Plus, Share, X, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function AdminPage() {
	...
	const filteredUsers = [
		...(filteredTechnicians || []).map(t => ({ ...t, role: "technician" as const })),
		...(filteredDispatchers || []).map(d => ({ ...d, role: "dispatcher" as const })),
	].sort((a, b) => a.name.toLowerCase().localeCompare(b.name.toLowerCase()));

	const displayUsers = filteredUsers.filter(u =>
		(u.role === "technician" && showTechnicians) ||
		(u.role === "dispatcher" && showDispatchers)
	);
    ...
    return (
        <div className="text-white">
			...
			{/* Possibly add a Pagination Header here for user ease*/}
           ...
           {/*!isFetchLoading && !fetchError && filteredDispatchers && filteredDispatchers.length > 0 && showDispatchers && (
